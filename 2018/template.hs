...
apply :: Op -> Int -> Int -> Int -- add mul eq gtr
apply Add a b = a + b
apply Mul a b = a * b
apply Eq a b
    | a == b = 1
...
eval (Const n) _ = n
eval (Var x) sts = lookUp x sts
eval (Apply op exp1 exp2) sts = apply op (eval exp1 sts) (eval exp2 sts)

execStatement :: Statement -> State -> State
execStatement (Assign id exp) sts = update (id, eval exp sts) sts
execStatement (If exp block1 block2) sts 
    | eval exp sts == 1 = execBlock block1 sts
    | otherwise =  execBlock block2 sts
execStatement (DoWhile block exp) sts 
    | eval exp (execBlock block sts) == 1 = execStatement (DoWhile block exp) (execBlock block sts)
    | otherwise = execBlock block sts

execBlock :: Block -> State -> State
execBlock block state  = foldl (flip execStatement) state block
